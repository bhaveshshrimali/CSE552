function [ElemK, ElemF,fint,ElemM] = Elast2d_Elem(xl,mateprop,nel,ndf,stress,Fbody,ul)
%
...
% Set Material Properties for the nonlinear material
mu1= mateprop(1);
mu2 = mateprop(2);
kappa = mateprop(3);
thick = mateprop(4);
...
for l = 1:lint
    ...
    for xi=1:length(F)
        for xj=1:length(F)
            for xk=1:length(F)
                for ll=1:length(F)
                    c(xi,xj,xk,ll)=(-mu1+(kappa+mu1)*(2*J-1))*Identity(xi,xj)*Identity(xk,ll)+4/J*mu2*FN(xi)*FN(xj)*FN(xk)*FN(ll)+...
                        (mu1-(kappa+mu1)*(J-1))*(Identity(xi,ll)*Identity(xj,xk)+Identity(xi,xk)*Identity(ll,xj));
                end
            end
        end
    end
    ...
    sigma=[stress(l,1) stress(l,3);
        stress(l,3) stress(l,2)];
    
    GP=Bmat2'*sigma*Bmat2        ;                                          %geometric part
    
    GP_mat=[eye(2)*GP(1,1) eye(2)*GP(1,2) eye(2)*GP(1,3) eye(2)*GP(1,4);    %terms only get added to the diagonal
    ...
    fint = fint+W*Bmat'*stress(l,:)' ; %Computing interal forces in the element
