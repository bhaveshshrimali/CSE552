function [ElemK, ElemF,fint,ElemM,Un] = Elast2d_Elem(xl,mateprop,nel,ndf,ndm,PSPS,stress,strain,Fbody)
%
...
thick = mateprop(3);
rho = mateprop(4);
% tr_epsilon = strain(1,1)+strain(2,1);

%Note: We use the same D Matrix for both the loading condition in (a) & (b)
...
for l = 1:lint
    ...
    treps = strain(l,1) + strain(l,2);
    treps2 = strain(l,1)^2 + strain(l,2)^2 + 0.5*strain(l,3)^2;
    A=a*((2+treps)/(1+treps)^2);
    Dmat = [(A+3*b)            A                  0;
        A                  A+3*b              0;
        0                  0                  3/2*b];
...
fint = fint + W*Bmat'*[stress(l,1) stress(l,2) stress(l,3)]';
ElemM = ElemM + rho*W*(shpm')*(shpm);
ElemF = ElemF + W*shpm'*Fbody;
Un = Un + W * (a*treps*log(1+treps) + 3*b/2*treps2);
end
end
