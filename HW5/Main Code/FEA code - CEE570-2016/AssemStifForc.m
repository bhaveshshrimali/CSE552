...
for ie1 = 1:nel
    for l1 = 1:ndf
        ...
        grow = EDOFT(locind1);
        if grow <= neq
            Fd(grow) = Fd(grow) + ElemF(locind1);
            
            %%%%%%%%%%%%%%%% MODIFICATION %%%%%%%%%%%%%%%%%%%%%%%%
            
            F_bar_int (grow) = F_bar_int (grow) + fint(locind1);
            ...
            for ie2 = 1:nel
                for l2 = 1:ndf
                    ...
                    gcol = EDOFT(locind2);
                    if gcol <= neq
                        Kdd(grow, gcol) = Kdd(grow, gcol) + ElemK(locind1,locind2);
                        Mdd(grow, gcol) = Mdd(grow, gcol) + ElemM(locind1,locind2);
                    else %gcol > neq
                        gcol = gcol - neq;
                        Kdf(grow, gcol) = Kdf(grow, gcol) + ElemK(locind1,locind2); 
            ...
            for ie2 = 1:nel
                for l2 = 1:ndf
                    ...
                    gcol = EDOFT(locind2);
                    if gcol <= neq
                        Kfd(grow, gcol) = Kfd(grow, gcol) + ElemK(locind1,locind2);
                      ...
                      %%%%%%%%%%%%%%%% MODIFICATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
                        ...
                        gcol = gcol - neq;
                        Kff(grow, gcol) = Kff(grow, gcol) + ElemK(locind1,locind2);
